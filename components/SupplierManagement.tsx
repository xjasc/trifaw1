...
const SupplierManagement: React.FC<SupplierManagementProps> = ({ suppliers = [], onSaveSupplier, onDeleteSupplier }) => {
  const [showModal, setShowModal] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);

  const [formData, setFormData] = useState({
    name: '', category: 'materials', email: '', phone: '', document: '', city: '', state: ''
  });

  const handleOpenCreate = () => {
    setEditingId(null);
    setFormData({ name: '', category: 'materials', email: '', phone: '', document: '', city: '', state: '' });
    setShowModal(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    await onSaveSupplier(newSup);
    setShowModal(false);
  };

  const deleteSupplier = async (id: string) => {
    if (window.confirm("Atenção: Excluir este fornecedor permanentemente do catálogo?")) await onDeleteSupplier(id);
  };
              ...
              <form onSubmit={handleSave} className="space-y-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input type="text" placeholder="(XX) XXXXX-XXXX" value={formData.phone} maxLength={15} onChange={e => setFormData({ ...formData, phone: formatPhone(e.target.value) })} className={inputClass} />
                  </div>
                </div>
