...
import { Project, Expense, ExpenseStatus, ProjectStatus, User, UserRole, Attachment, Measurement, Supplier, PhotoTopic, ClientData, ResponsibleData } from '../types';
...
const ProjectDetails: React.FC<ProjectDetailsProps> = ({ project, users = [], suppliers = [], onUpdateProject, currentUser }) => {
  ...
  const [formData, setFormData] = useState({
    description: '', amount: 0, category: CATEGORIES[0], supplier: '', status: ExpenseStatus.REALIZED, date: new Date().toISOString().split('T')[0], attachmentUrl: ''
  });
  ...
  // Listas Ordenadas
  const sortedExpenses = useMemo(() => [...(project.expenses || [])].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()), [project.expenses]);
  const sortedMeasurements = useMemo(() => [...(project.measurements || [])].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()), [project.measurements]);
  ...
  const filteredList = useMemo(() => {
    ...
    return sourceList.filter(e => e.date.startsWith(filterMonth));
  ...
  const projectStats = useMemo(() => {
    const realizedExpenses = sortedExpenses.filter(e => e.status === ExpenseStatus.REALIZED).reduce((acc, e) => acc + (Number(e.amount) || 0), 0);
    const futureExpenses = sortedExpenses.filter(e => e.status === ExpenseStatus.FUTURE).reduce((acc, e) => acc + (Number(e.amount) || 0), 0);
    
    const realizedBilling = measurements.filter(m => m.status === ExpenseStatus.REALIZED).reduce((acc, m) => acc + (Number(m.amount) || 0), 0);
    const futureBilling = measurements.filter(m => m.status === ExpenseStatus.FUTURE).reduce((acc, m) => acc + (Number(m.amount) || 0), 0);
      ...
      <div className="bg-[#fafaf9] p-6 rounded-[2rem] shadow-sm border border-[#e7e5e4] flex flex-col gap-4">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start gap-4">
           ...
           <div className="flex flex-col items-end gap-2 shrink-0">
             <div className={`px-4 py-2 text-white rounded-xl shadow-sm ${
                project.status === ProjectStatus.ACTIVE ? 'bg-emerald-950' : 
        ...
        <div className="bg-white p-4 rounded-2xl border border-stone-200">
           <div className="flex justify-between items-center px-1 mb-2">
              <span className="text-[8px] font-black uppercase text-stone-400 tracking-widest">Avanço Físico</span>
              <span className="text-lg font-black text-emerald-900 italic">{project.physicalProgress || 0}%</span>
           </div>
           <div className="h-3 bg-stone-100 rounded-full overflow-hidden relative border border-stone-200">
             <div className="h-full bg-emerald-800 rounded-full transition-all duration-1000" style={{ width: `${project.physicalProgress || 0}%` }}></div>
             {isAdmin && <input type="range" min="0" max="100" value={project.physicalProgress || 0} onChange={(e) => onUpdateProject({...project, physicalProgress: parseInt(e.target.value)})} className="absolute inset-0 w-full opacity-0 cursor-pointer" />}
           </div>
        </div>
      </div>
                ...
                <div className="flex items-center gap-2 shrink-0 w-full md:w-auto px-1">
                    ...
                    <button 
                        onClick={() => { 
                            ...
                            setFormData({description:'', amount:0, category:CATEGORIES[0], supplier:'', status: ExpenseStatus.REALIZED, date: new Date().toISOString().split('T')[0], attachmentUrl: ''}); 
                            setExpenseFile(null);
                ...
                <div className="flex items-center gap-2 shrink-0 w-full md:w-auto px-1">
                    ...
                    <button 
                        onClick={() => { 
                            ...
                            setFormData({description:'', amount:0, category:'', supplier:'', status: ExpenseStatus.REALIZED, date: new Date().toISOString().split('T')[0], attachmentUrl: ''}); 
                            setExpenseFile(null);
                            setShowEntryModal(true); 
                        }} 
                        className="flex-1 md:flex-none px-4 py-2 bg-emerald-950 text-white font-black uppercase text-[9px] rounded-xl shadow-md hover:bg-emerald-900 transition-all active:scale-95 flex items-center justify-center gap-2 tracking-wide"
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div className="relative">
                                <select className={selectClass} value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})}>
                                    {CATEGORIES.map(c => <option key={c} value={c} className="text-stone-800">{c}</option>)}
                                </select>
                                <i className="fa-solid fa-chevron-down absolute right-4 top-1/2 -translate-y-1/2 text-stone-400 text-[10px] pointer-events-none"></i>
                            </div>
                        </div>
